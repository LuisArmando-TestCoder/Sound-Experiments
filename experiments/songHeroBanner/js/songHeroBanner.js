(function() {
  ...
  function animate() {
    ...
    let barIndex = 0;
    const _size = 7, _spacing = 2;
    updateShapes(audioBars, (obj) => {
      obj.x = (c.width / 2) + barIndex * (_size + _spacing) - barsAmount * _size / 2 - _spacing * barsAmount / 2;
      obj.y = c.height / 2;
      if(songContext) {
  ...
  const circles = [];
  const audioBars = [];
  const nodesAmount = 89, barsAmount = 21;
  ...
  repeat(barsAmount, (i) => {
    const _size = 7, _spacing = 2;
    create(audioBars, {
      x: (c.width / 2) + i * (_size + _spacing) - barsAmount * _size / 2 - _spacing * barsAmount / 2,
      y: c.height / 2,
      width: _size,
      height: songContext,
